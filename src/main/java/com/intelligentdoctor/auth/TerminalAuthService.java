...
public class TerminalAuthService {
    ...
    public TerminalAuthService(AdminUserRepository adminUserRepository,
    ...
    public AdminLoginResponse login(String username, String password) {
        ...
        AdminUserEntity user = adminUserRepository.findByHospitalIdAndUsernameAndRole(hospitalId, normalizedUsername, "TERMINAL_USER")
                .orElseThrow(() -> new IllegalArgumentException("导诊终端账号或密码不正确"));
        if (!Boolean.TRUE.equals(user.getEnabled()) || !passwordService.matches(password, user.getPasswordHash())) {
            throw new IllegalArgumentException("导诊终端账号或密码不正确");
        }
        AdminPrincipal principal = new AdminPrincipal(hospitalId, user.getUsername(), "TERMINAL_USER");
